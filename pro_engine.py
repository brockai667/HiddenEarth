#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""PRO engine pre HiddenEarth reels (overeny na Plitvice demu, schvaleny 2026-07-04).
...
def chip_png(text, size=58, kind="orange", font=FONT_ANT, tracking=2):
    ...
    if kind == "orange":
        ...
        for y in range(h):
            t = y / max(1, h - 1)
            c = tuple(int(ORANGE[i] * (1 - t) + ORANGE2[i] * t) for i in range(3)) + (255,)
            grad.putpixel((0, y), c)
    ...
    if role == "hook":
        ...
        for ci, c in enumerate((sc.get("chips") or [])[:2]):
            chips[ci] = {"img": chip_png(str(c.get("t", ""))[:24].upper(), 62 if ci else 64,
                                         c.get("style", "orange" if ci else "white")),
                         "at": word_time(sc, c.get("on"), 0.5 + 1.5 * ci)}
    elif role == "callout":
        chips[0] = {"img": chip_png(str(sc.get("label", ""))[:24].upper(), 60, "orange"),
                    "at": word_time(sc, sc.get("label_on") or sc.get("punch"), 1.2)}
        if sc.get("sub"):
            chips[1] = {"img": chip_png(str(sc["sub"])[:34], 42, "glass", FONT_POP),
                        "at": chips[0]["at"] + 0.25}
    elif role == "cta":
        chips[0] = {"img": chip_png("@HiddenEarth", 66, "orange"), "at": 0.35}
            ...
            for li, ln in enumerate(lines):
                ...
                for (wtxt, col, wwd) in ln:
                    ...
                    if t >= t0:
                        ...
                        fill = ORANGE + (255,) if col == "o" else (255, 255, 255, 255)
...
def render_map_frames(sc, spec, idx, work):
    ...
    label = chip_png(str(spec.get("place", ""))[:22].upper(), 62, "orange")
    ...
    for fi in range(n):
        ...
        dd.ellipse((pxc - rr, pyc - rr, pxc + rr, pyc + rr),
                   outline=ORANGE + (int(200 * (1 - ph)),), width=6)
...
# ---------------------------------------------------------------- render scen
GRADE = "eq=contrast=1.06:saturation=1.14:brightness=0.01,vignette=angle=PI/5,noise=alls=4:allf=t"
